...
for filename in start_dir.glob("**/*.pdf"):
    ...
    if ocrmypdf.pdfa.file_claims_pdfa(filename)["pass"]:
        ...
        if archive_dir:
            # Join the archive directory with the file's relative path
            archive_filename = Path(archive_dir) / filename.relative_to(start_dir)
            logging.info(f"Archiving document to {archive_filename}")
            if not filecompare(filename, archive_filename):
